...
class SnakeEnv:
    def __init__(self, mode='TEST', grid_size=10, block_size=20):
        """初始化游戏环境"""
    ...
    def get_env_info(self):
        """获取环境信息，包括状态大小和动作大小"""
        input_channels = 1
        state_size = self.grid_size * self.grid_size  # 状态信息大小：食物位置和蛇头位置
    ...
    def _get_state(self):
        ...
        game_state[snake_head[0]][snake_head[1]] = 3
    ...
    def step(self, action):
        ...
        if not self.done:
            ...
            if new_head == self.food_pos:  # 吃到食物
                self.score += 1
                self.food_pos = self._generate_food()  # 生成新的食物
            else:
                self.snake_pos.pop()  # 移动蛇，未吃到食物则移除尾部

        # 引导，向食物靠近则+0.01，远离则-0.01
        # 计算蛇头移动前后与食物的曼哈顿距离差异
        ...
        # 如果蛇头移动后与食物的曼哈顿距离减少（即蛇向食物靠近），则增加分数
        # 否则，如果蛇头移动后与食物的距离增加或保持不变（即蛇未向食物靠近），则减少分数
